...
namespace deovrScraper.Services
{
  public class EpornerScraper(ILogger<EpornerScraper> logger, IServiceProvider serviceProvider, IConfiguration config, IVideoService vs) : IEpornerScraper
  {
    ...
    */
    ...
    public async Task<VideoSource?> GetSource(DbVideoItem video, DeovrScraperContext context)
    {
    ...
    public async Task ParseDetails(DbVideoItem video, DeovrScraperContext context)
    {
      var details = await this.GetDetails(video);
