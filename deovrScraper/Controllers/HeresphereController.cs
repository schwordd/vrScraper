using deovrScraper.Controllers.Models.HereSphere;
...
namespace deovrScraper.Controllers
{
  [Route("[controller]")]
  [ApiController]
  public class HeresphereController(ILogger<HeresphereController> logger, IEpornerScraper scraper, DeovrScraperContext context, IVideoService videoService, IConfiguration config, ISettingService settings) : ControllerBase
  {
    ...
    public async Task<dynamic> Post()
    {
      ...
      tabConfigs.ForEach(t =>
      {
        switch (t.Type)
        {
          case "DEFAULT":
            if (t.Name == "Latest")
            {
              var list1 = allItems.OrderByDescending(v => Convert.ToInt32(v.SiteVideoId)).Select(item => $"http://{config["Ip"]}:{config["Port"]}/heresphere/{item.Id}").ToList<string>();
              tabs.Add((t.Name, list1));
            }
            else if (t.Name == "Rating")
            {
              var list2 = allItems.OrderByDescending(v => v.SiteRating).ThenByDescending(v => v.Views).Select(item => $"http://{config["Ip"]}:{config["Port"]}/heresphere/{item.Id}").ToList<string>();
              tabs.Add((t.Name, list2));
            }
            else if (t.Name == "Random")
            {
              var list3 = allItems.OrderBy(a => Guid.NewGuid()).Select(item => $"http://{config["Ip"]}:{config["Port"]}/heresphere/{item.Id}").ToList<string>();

              tabs.Add((t.Name, list3));
            }
            else if (t.Name == "Fav")
            {
              var list4 = allItems.Where(x => x.Favorite == true).OrderBy(a => Guid.NewGuid()).Select(item => $"http://{config["Ip"]}:{config["Port"]}/heresphere/{item.Id}").ToList<string>();

              tabs.Add((t.Name, list4));
            }
            else if (t.Name == "Liked")
            {
              var list5 = allItems.Where(x => x.Liked == true).OrderBy(a => Guid.NewGuid()).Select(item => $"http://{config["Ip"]}:{config["Port"]}/heresphere/{item.Id}").ToList<string>();

              tabs.Add((t.Name, list5));
            }
            else if (t.Name == "Playtime")
            {
              var list6 = allItems.OrderByDescending(a => a.PlayDurationEst).Where(a => a.PlayDurationEst > TimeSpan.FromSeconds(20)).Select(item => $"http://{config["Ip"]}:{config["Port"]}/heresphere/{item.Id}").ToList<string>();
            ...
            else if (t.Name == "Unwatched")
            {
              ...
              var list7 = allUnwatched.OrderByDescending(a =>
                  ...
                  ).Select(item => $"http://{config["Ip"]}:{config["Port"]}/heresphere/{item.Id}").ToList<string>();
      ...
      return new
      {
        ...
        banner = new
        {
          image = $"http://{config["Ip"]}:{config["Port"]}/logo.png",
          link = $"http://{config["Ip"]}:{config["Port"]}/heresphere"
        },
    ...
    public async Task<dynamic> Detail(int videoId, HereSphereGetDetailsModel model)
    {
      ...
      return new
      {
        ...
        media = source == null ? new List<dynamic>() : new List<dynamic>()
        {
           new {
                  name = "h264",
